...
class CCAvenueSettings(Document):
	...
	def get_payment_payload(self, **kwargs):
		...
		ccavenue_params = get_ccavenue_params(kwargs, integration_request.name, ccavenue_config)
...
def get_ccavenue_config():
	...
	ccavenue_config.update(
		dict(working_key=get_decrypted_password("CCAvenue Settings", "CCAvenue Settings", "working_key"))
	)

	if cint(ccavenue_config.sandbox):
		ccavenue_config.update(
			dict(url="https://test.ccavenue.com/transaction/transaction.do?command=initiateTransaction")
		)
	else:
		ccavenue_config.update(
			dict(url="https://secure.ccavenue.com/transaction/transaction.do?command=initiateTransaction")
		)
...
def verify_transaction(**kwargs):
	...
	try:
		plain_response = dict(parse_qsl(decrypt(enc_response, ccavenue_config.working_key)))
	except Exception:
	...
	log_webhook_response(
		order_id=order_id,
		status=("Success" if plain_response.get("order_status") in CCAVENUE_SUCCESS_STATUSES else "Failed"),
		decrypted_response=plain_response,
	)
...
def finalize_request(order_id, transaction_response):
	...
	transaction_data = request.data and frappe.parse_json(request.data) or frappe._dict()
	...
	if is_pending:
		...
		try:
			...
			custom_redirect_to = reference_doc.run_method("on_payment_authorized", payment_status)
			if is_success:
				status = "Completed"
	...
	if redirect_to:
		...
		if redirect_message:
			...
			redirect_url += separator + urlencode({"redirect_message": redirect_message})
	else:
		redirect_url = get_url("payment-success" if status == "Completed" else "payment-failed")
		if redirect_message:
...
def _handle_ccavenue_den_notification(error_log_context):
	...
	if not enc_response:
		frappe.log_error(f"{error_log_context} payload missing 'encResp'", "CCAvenue Webhook Error")
	...
	try:
		...
		frappe.log_error(frappe.get_traceback(), f"{error_log_context} Decryption Failed")
		frappe.local.response["http_status_code"] = 400
		log_webhook_response(
			status="Error",
			http_status_code=400,
	...
	try:
		...
		frappe.log_error(frappe.get_traceback(), f"{error_log_context} Processing Failed")
	...
	if transaction_data := (request.data and frappe.parse_json(request.data) or frappe._dict()):
		if transaction_data.reference_doctype and transaction_data.reference_docname:
			...
			try:
				...
				frappe.log_error(frappe.get_traceback(), "Webhook Document Hook Exception")
...
def get_logged_payment_attempts(order_id: str) -> list[dict]:
	...
	for log in logs:
		try:
			data = frappe.parse_json(log.decrypted_response) if log.decrypted_response else {}
		except Exception:
			data = {}
...
def get_order_lookup_attempts(order_id: str) -> list[dict]:
	...
	creation = get_datetime(
		frappe.db.get_value("Integration Request", order_id, "creation") or now_datetime()
	)
	...
	try:
		...
		decrypted_str = decrypt(response_dict["enc_response"].strip(), ccavenue_config.working_key)
...
def check_payment_status_by_id(order_id: str) -> dict:
	...
	attempts_by_key = {
		(a.get("tracking_id") or a.get("webhook_log")): a for a in get_logged_payment_attempts(order_id)
	}
	...
	try:
		...
		frappe.log_error(frappe.get_traceback(), "CCAvenue Status Query Exception Failed")
		return {
			"status": "Error",
...
def verify_pending_payments():
	...
	for request in pending_requests:
		...
		try:
			previous_output = (
				frappe.parse_json(frappe.db.get_value("Integration Request", order_id, "output") or "{}")
				or {}
			)
		except Exception:
			previous_output = {}
